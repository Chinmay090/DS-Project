...
/**
 * @brief Remove all currently stored pages for a table
 *
 * @param pageName 
 * @return Page 
 */
...
/**
 * @brief Renames pages when matrix or table is renamed.
 *
 * @param fromTableName 
 * @param toTableName 
 */
void BufferManager::renamePages(string fromTableName, string toTableName) 
{
    logger.log("BufferManager::renamePages");
    string directoryPath = "../data/temp/";
    string searchPage = fromTableName + "_Page";
    for (const auto& entry : std::filesystem::directory_iterator(directoryPath)) {
        if (std::filesystem::is_regular_file(entry)) {
            string currentFilePath = entry.path().string();
            string newFilePath = std::regex_replace(currentFilePath, std::regex(fromTableName), toTableName);
